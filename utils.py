...
def to_one_hot(tensor, n, fill_with=1.0):
    ...
    one_hot = torch.zeros(tensor.size() + (n,), device=tensor.device, dtype=torch.float)
    # if tensor.is_cuda:
...
def sample(model, sample_batch_size, obs, sample_op, labels=None):
    ...
    with torch.no_grad():
        ...
        if labels is None:
            labels = torch.zeros(sample_batch_size, dtype=torch.long, device=device)
...
def save_images(tensor, images_folder_path, label=""):
    ...
    for i, img_tensor in enumerate(tensor):
        img = Image.fromarray(
            (img_tensor.cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8),
            mode="RGB",
        )

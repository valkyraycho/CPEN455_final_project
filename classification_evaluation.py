...
def get_label(model, model_input, device):
    ...
    for c in range(num_classes):
        class_tensor = torch.full((batch_size,), c, dtype=torch.long, device=device)

        model_output = model(model_input, class_tensor, sample=False)

        for i in range(batch_size):
            x_i = model_input[i : i + 1]  # single image (1, C, H, W)
            out_i = model_output[i : i + 1]
            loss = discretized_mix_logistic_loss(x_i, out_i)
            losses[i, c] = loss

    predicted_labels = torch.argmin(losses, dim=1)
